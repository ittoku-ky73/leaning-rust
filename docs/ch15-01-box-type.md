...
コンパイラは再帰的に定義された型に必要なメモリ量の計算ができないので、エラーを返します。
しかしエラーにはこのような役立つ提案が含まれています。
...
この提案の間接参照は、値を直接格納するのではなく、データ構造を変更して値を間接的に格納することを意味します。
これは値の代わりに値へのポインタを格納することによって可能になります。

`Box<T>`はポインタなのでコンパイラには`Box<T>`が必要とする領域がわかります。
すなわち、ポインタのサイズは指しているデータの量に左右されません。
つまり別の`List`値を直接置く代わりに、`Cons`列挙子の中に`Box<T>`を配置することができます。

`Box<T>`は、`Cons`列挙子の中ではなく、ヒープに置かれる次の`List`値を指します。
概念的には依然としてリストは他のリストを保持するリストによって作られたものです。
しかし今やこの実装は、要素をお互いの中に配置するというより、隣り合うように配置するような感じになります。

以下のコードは上記のコードに変更を加えたものです。このコードはコンパイルすることができます。
...
`Cons`は1つの`i32`とボックスのポインタデータを格納する領域を必要とします。
`Nil`は値を格納しないので`Cons`よりも必要な領域は小さくなります。
これで`List`は、`i32`とボックスのポインタデータのサイズを必要になるかわかるようになります。

ボックスを使用することで無限に続く再起の連鎖を断ち切ることができます。
これによってコンパイラは、`List`を格納するのに必要なサイズを計算することができます。

![Cons(i32, Box<List>)](https://doc.rust-jp.rs/book-ja/img/trpl15-02.svg) 

ボックスは間接参照とヒープメモリ確保だけを提供します。
他のスマートポインタ型に見られるような別の特別な能力はありません。
これらの特別な能力が招くパフォーマンスのオーバーヘッドもないので、コンスリストのように間接参照だけが必要な機能である場合には便利でしょう。
こボックスのユースケースは第17章で見ることができます。

`Box<T>`型がスマートポインタなのは、`Deref`トレイトを実装しているからです。
このトレイトにより`Box<T>`の値を参照のように扱うことができます。
`Box<T>`がスコープを抜けると、ボックスが参照しているヒープデータも片付けられます。
これは`Drop`トレイト実装によるものです。

これら2つのトレイトをこれからより詳しく探求していきましょう。
これら2つは他のスマートポインタ型が提供する機能にとってさらに重要なものです。
